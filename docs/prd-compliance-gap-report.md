...
#### Gap 4. Countdown timer / reservation UX — PARTIAL
PRD story: 10

Current state:
- `reservedUntil` is now exposed in every order response ([OrderResponse.java](../src/main/java/com/example/inventory/application/dto/OrderResponse.java)), so a frontend can render a countdown.

Still missing:
1. A remaining-time projection/countdown payload immune to client clock skew.
2. A push/event notification when the reservation expires mid-checkout (WebSocket infrastructure exists in [OrderWebSocketService.java](../src/main/java/com/example/inventory/infrastructure/websocket/OrderWebSocketService.java) but is not wired to reservation expiry).
